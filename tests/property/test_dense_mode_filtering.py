...
def empty_data_strategy(draw):
    ...
    return draw(st.one_of(
        ...
        st.dictionaries(
            ...
            st.one_of(st.none(), st.just(""), st.just([]), st.just({})),
            min_size=1, max_size=3
        ),
        st.lists(st.one_of(st.none(), st.just(""), st.just([]), st.just({})), min_size=1, max_size=3)
    ))
...
class TestDenseModeFiltering:
    ...
    def test_dense_mode_excludes_empty_data(self, result: AnalysisResult):
        ...
        **Validates: Requirements 4.2**
        ...
        if not formatter._has_meaningful_data(result.classifications):
            assert 'classifications' not in parsed or not formatter._has_meaningful_data(parsed.get('classifications')), \
                "Empty classifications should be filtered out in dense mode"
        
        if not formatter._has_meaningful_data(result.local_info):
            assert 'local_info' not in parsed or not formatter._has_meaningful_data(parsed.get('local_info')), \
                "Empty local_info should be filtered out in dense mode"
        
        if not formatter._has_meaningful_data(result.internet_info):
            assert 'internet_info' not in parsed or not formatter._has_meaningful_data(parsed.get('internet_info')), \
                "Empty internet_info should be filtered out in dense mode"
        
        if not formatter._has_meaningful_data(result.application_info):
            assert 'application_info' not in parsed or not formatter._has_meaningful_data(parsed.get('application_info')), \
                "Empty application_info should be filtered out in dense mode"
        
        if not formatter._has_meaningful_data(result.errors):
            assert 'errors' not in parsed or not formatter._has_meaningful_data(parsed.get('errors')), \
                "Empty errors should be filtered out in dense mode"
    ...
    def test_dense_vs_full_mode_difference(self, data):
        ...
        **Validates: Requirements 4.2**
        ...
        result = data.draw(analysis_result_with_controlled_data(has_meaningful_data=data.draw(st.booleans())))
    ...
    def test_dense_mode_consistency_across_formatters(self, result: AnalysisResult):
        ...
        **Validates: Requirements 4.2**
        ...
        assert set(json_filtered.keys()) == set(html_filtered.keys()) == set(human_filtered.keys()), \
            "All formatters should filter the same fields in dense mode"
