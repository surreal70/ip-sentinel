...
def scan_results(draw):
    ...
    scan_duration_ms = draw(
        st.integers(
            min_value=1,
            max_value=300000))  # 1ms to 5 minutes
    ...
    for module_name in executed_modules:
        ...
        if success:
            ...
            if module_name == "classification":
                ...
                data = {
                    "reachable": draw(
                        st.booleans()), "mac_address": draw(
                        st.one_of(
                            st.none(), st.text(
                                min_size=17, max_size=17))), "ports": draw(
                        st.lists(
                            st.integers(
                                min_value=1, max_value=65535), max_size=10))}
    ...
    def test_scan_result_storage_completeness(self, scan_result):
        ...
        **Validates: Requirements 3.1, 3.2, 3.5**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert isinstance(
                scan_id, int), "store_scan_result should return an integer scan ID"
            ...
            for module_name in scan_result["modules_executed"]:
                ...
                if original_module["success"]:
                    ...
                    # Data should be equivalent (allowing for JSON serialization
                    # differences)
        ...
        **Validates: Requirements 3.5**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for scan_id in stored_scan_ids:
                ...
                # This test ensures the system is capturing and preserving timestamp
                # information
                assert isinstance(
    stored_dt, datetime) or isinstance(
         stored_timestamp, str), "Stored timestamp should be a datetime object or ISO string"
