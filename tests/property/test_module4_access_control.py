...
class TestModule4AccessControl:
    ...
    def test_module4_requires_explicit_submodule_specification(
            self, submodule_names, ip_address):
        """
    ...
    def test_module4_validates_submodule_availability(
            self, valid_submodules, invalid_submodules):
        """
        ...
        **Validates: Requirements 5.4, 5.5**
        ...
        if valid_submodules:
            ...
            for submodule_name in valid_submodules:
                assert availability[
                    submodule_name] is True, f"Valid submodule {submodule_name} should be available"
        ...
        for submodule_name in invalid_submodules:
            assert availability[
                submodule_name] is False, f"Invalid submodule {submodule_name} should not be available"
    ...
    @given(submodule_name=st.sampled_from(['netbox',
                                           'checkmk', 'openitcockpit', 'openvas', 'infoblox']))
    ...
    def test_module4_no_default_execution(self, ip_address):
        ...
        **Validates: Requirements 5.1, 5.2**
        ...
        assert len(
            app_module.loaded_submodules) == 0, "No submodules should be loaded by default"
