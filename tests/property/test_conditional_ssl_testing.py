...
class TestConditionalSSLTesting:
    ...
    def test_ssl_testing_for_web_and_mail_ports(self, ip, ports):
        ...
        **Validates: Requirements 7.11, 7.12**
        ...
        with patch.dict('sys.modules', {
            ...
            mock_results = []
            expected_ssl_ports = [
                p for p in ports if p in ssl_eligible_ports or self._is_ssl_service(p)]
            ...
            if expected_ssl_ports:
                assert len(
                    ssl_results) > 0, "SSL analysis should be performed for web/mail server ports"
                ...
                for ssl_result in ssl_results:
                    assert isinstance(
                        ssl_result, SSLResult), "Result must be SSLResult instance"
                    ...
                    # Verify that web server ports (Requirements 7.11) and mail server
                    # ports (Requirements 7.12) are tested
                ...
                assert len(
                    ssl_results) == 0, "No SSL analysis should be performed for non-eligible ports"
    ...
    def test_ssl_testing_skipped_for_non_eligible_ports(self, ip):
        ...
        # SSH, FTP, Telnet, DNS, RDP, PostgreSQL, MySQL
        non_ssl_ports = [22, 21, 23, 53, 3389, 5432, 3306]
        ...
        assert len(
            ssl_results) == 0, "SSL analysis should return empty results for non-eligible ports"
...
class TestSSLAnalysisErrorHandling:
    ...
    def test_ssl_analysis_with_connection_failures(self, ip, ssl_ports):
        ...
        with patch.dict('sys.modules', {
            ...
            assert isinstance(
                ssl_results, list), "SSL analysis should return list even on failures"

            # If results are returned, they should contain error information
            for result in ssl_results:
                assert isinstance(
                    result, SSLResult), "Failed results should still be SSLResult instances"
