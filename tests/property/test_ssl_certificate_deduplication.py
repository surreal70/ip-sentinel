...
def certificate_data(draw):
    ...
    return {
        ...
        'fingerprint': f'sha256:{"a" * 10}{cert_id:02d}{"b" * 52}'
    }
...
class TestSSLCertificateDeduplication:
        ...
        **Validates: Requirements 7.13**
        ...
        with patch.dict('sys.modules', {
            ...
            for port in ssl_ports:
                ...
                mock_cert_info.certificate_deployments[0].received_certificate_chain = [
                    MagicMock()]
            ...
            if ssl_results:
                ...
                self._group_certificates_by_identity(ssl_results)

                # Verify deduplication behavior
                unique_certificates = set()
                for result in ssl_results:
                    if result.certificate:
                        cert_identity = self._get_certificate_identity(
                            result.certificate)
                ...
                # If we have multiple ports with the same certificate, verify
                # deduplication
                ports_per_cert = {}
                for port in ssl_ports:
                    cert_index = port_cert_mapping[port]
                ...
                duplicate_cert_groups = {
                    k: v for k, v in ports_per_cert.items() if len(v) > 1}

                if duplicate_cert_groups:
                    # Verify that identical certificates are handled appropriately
                    # The requirement is that they should be "reported once" with port
                    # differences documented

                    for cert_index, ports_with_cert in duplicate_cert_groups.items():
                        # Find SSL results for these ports
                        results_for_cert = [
                            r for r in ssl_results if r.port in ports_with_cert]

                        if results_for_cert:
                            # Separate primary and reference results
                            primary_results = [
                                r for r in results_for_cert if 'reference_to_port' not in r.certificate]
                            reference_results = [
                                r for r in results_for_cert if 'reference_to_port' in r.certificate]

                            # Should have exactly one primary result for identical
                            # certificates
    ...
    def test_different_certificates_reported_separately(self, ip):
        ...
        with patch.dict('sys.modules', {
            ...
            if ssl_results:
                ...
                # Should have as many unique subjects as ports (since all certs are
                # different)
    ...
    def test_certificate_deduplication_with_port_differences(self, ip, ssl_ports):
        ...
        with patch.dict('sys.modules', {
            ...
            if ssl_results and len(ssl_results) > 1:
                ...
                primary_results = [
                    r for r in ssl_results if 'reference_to_port' not in r.certificate]
                reference_results = [
                    r for r in ssl_results if 'reference_to_port' in r.certificate]

                # Should have at least one primary result
                assert len(
                    primary_results) >= 1, "Should have at least one primary certificate result"
                ...
                if len(primary_results) > 1:
                    ...
                    for result in primary_results[1:]:
                        if result.certificate and first_cert:
                            assert result.certificate.get('subject') == first_cert.get(
                                'subject'), "Identical certificates should have same subject"
                            assert result.certificate.get('serial_number') == first_cert.get(
                                'serial_number'), "Identical certificates should have same serial number"

                # Verify that port information is preserved (port-specific differences
                # documented)
                result_ports = {r.port for r in ssl_results}
                expected_ports = set(ssl_ports)
                assert result_ports == expected_ports, \
    ...
    def _group_certificates_by_identity(
            self, ssl_results: List[SSLResult]) -> Dict[str, List[SSLResult]]:
...
class TestCertificateDeduplicationErrorHandling:
    ...
    def test_deduplication_with_missing_certificate_data(self, ip, ssl_ports):
        ...
        with patch.dict('sys.modules', {
            ...
            assert isinstance(
                ssl_results, list), "Should return list even with mixed certificate data"

            # Results should be returned for all ports, even those with missing
            # certificates
            if ssl_results:
                {r.port for r in ssl_results}
                set(ssl_ports)
