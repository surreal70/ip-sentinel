...
def valid_classification_rules(draw):
    ...
    description = draw(st.text(min_size=1, max_size=200, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'Zs'), whitelist_characters='.-_')))
    ...
    rfc_reference = draw(st.one_of(st.none(), st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'), whitelist_characters=' -'))))
...
def valid_rule_names(draw):
    ...
    return draw(st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'), whitelist_characters='_-')))
...
class TestClassificationCRUDOperations:
    ...
    def test_add_classification_persists_to_json(self, rule):
        ...
        **Validates: Requirements 6.6**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert rule.name in loaded_rules, f"Added rule '{rule.name}' should be in loaded classifications"
    ...
    def test_delete_classification_removes_from_json(self, rule):
        ...
        **Validates: Requirements 6.6**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Verify it exists
            loaded_rules = config_manager.load_classifications()
            assert rule.name in loaded_rules, f"Rule '{rule.name}' should exist before deletion"
            ...
            assert rule.name not in loaded_rules_after, f"Deleted rule '{rule.name}' should not be in loaded classifications"
    ...
    def test_update_classification_modifies_json(self, original_rule, updated_rule):
        ...
        **Validates: Requirements 6.6**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Update the rule
            try:
                result = config_manager.update_classification(original_rule.name, updated_rule)
            except ValueError:
            ...
            if original_rule.name != updated_rule.name:
                assert original_rule.name not in loaded_rules, f"Original rule name '{original_rule.name}' should not exist after update"
            
            # Updated rule should exist
            assert updated_rule.name in loaded_rules, f"Updated rule '{updated_rule.name}' should exist"
    ...
    def test_update_nonexistent_classification_returns_false(self, nonexistent_name, updated_rule):
        """
        ...
        **Validates: Requirements 6.6**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            existing_rules = config_manager.load_classifications()
            assume(nonexistent_name not in existing_rules)  # Only test with non-existent names
            
            # Try to update non-existent rule
            try:
                result = config_manager.update_classification(nonexistent_name, updated_rule)
                assert result is False, f"Update of non-existent rule '{nonexistent_name}' should return False"
            except ValueError:
                assume(False)  # Skip invalid updated rules

    @given(st.lists(valid_classification_rules(), min_size=1, max_size=5, unique_by=lambda x: x.name))
        ...
        **Validates: Requirements 6.6**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for rule in added_rules:
                assert rule.name in loaded_rules, f"Added rule '{rule.name}' should exist"
            ...
            for rule in rules_to_delete:
                assert rule.name not in loaded_rules_after_delete, f"Deleted rule '{rule.name}' should not exist"
            
            remaining_rules = added_rules[len(added_rules)//2:]
            for rule in remaining_rules:
                assert rule.name in loaded_rules_after_delete, f"Remaining rule '{rule.name}' should still exist"