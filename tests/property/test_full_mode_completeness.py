    ...
    def test_full_mode_includes_all_sections(self, result: AnalysisResult):
        ...
        **Validates: Requirements 4.4, 4.5**
        ...
        for section in expected_sections:
            ...
            # If the original data was empty/None, it should show "no results" or be
            # present but empty
            original_value = getattr(result, section)
            if not formatter._has_meaningful_data(original_value):
                # In full mode, empty data should either be "no results" or the empty
                # structure
        ...
        for section_name, original_value in sections_to_check:
            if section_name in parsed:
                ...
                if not formatter._has_meaningful_data(original_value):
                    ...
                    is_empty_structure = not formatter._has_meaningful_data(
                        parsed_value)
    ...
    def test_full_mode_consistency_across_formatters(self, result: AnalysisResult):
        ...
        **Validates: Requirements 4.4, 4.5**
        ...
        assert set(json_filtered.keys()) == set(html_filtered.keys()) == set(
            human_filtered.keys()), "All formatters should include the same sections in full mode"
        ...
        for section in expected_sections:
            ...
            if any(
                json_formatter._has_meaningful_data(
                    getattr(
                        result,
                        attr,
                        None)) for attr in [
    ...
    def test_full_mode_preserves_meaningful_data(self, result: AnalysisResult):
        ...
        **Validates: Requirements 4.4, 4.5**
        ...
        if formatter._has_meaningful_data(result.classifications):
            assert 'classifications' in parsed and formatter._has_meaningful_data(
                parsed['classifications']), "Meaningful classifications should be preserved in full mode"

        if formatter._has_meaningful_data(result.local_info):
            assert 'local_info' in parsed and formatter._has_meaningful_data(
                parsed['local_info']), "Meaningful local_info should be preserved in full mode"

        if formatter._has_meaningful_data(result.internet_info):
            assert 'internet_info' in parsed and formatter._has_meaningful_data(
                parsed['internet_info']), "Meaningful internet_info should be preserved in full mode"

        if formatter._has_meaningful_data(result.application_info):
            assert 'application_info' in parsed and formatter._has_meaningful_data(
                parsed['application_info']), "Meaningful application_info should be preserved in full mode"

        if formatter._has_meaningful_data(result.errors):
            assert 'errors' in parsed and formatter._has_meaningful_data(
                parsed['errors']), "Meaningful errors should be preserved in full mode"
