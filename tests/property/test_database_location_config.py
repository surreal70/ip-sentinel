...
class TestDatabaseLocationConfiguration:
    ...
    @given(st.lists(valid_database_paths(), min_size=2, max_size=5, unique=True), sample_scan_data())
            ...
            for i, db_manager in enumerate(db_managers):
                ...
                for j, other_scan_id in enumerate(scan_ids):
                    if i != j:
                        ...
                        # This might be None or might be a different scan, but shouldn't be the same
