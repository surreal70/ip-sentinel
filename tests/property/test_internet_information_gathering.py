...
class TestInternetInformationGathering:
    ...
    def test_internet_information_gathering_completeness_property(self, ip: IPAddress):
        ...
        **Validates: Requirements 8.3, 8.4, 8.5, 8.6, 8.10**
        ...
        assert isinstance(result, dict), f"Internet analysis result should be a dictionary for IP {ip}"
        ...
        assert isinstance(whois_data, dict), f"WHOIS data should be a dictionary for IP {ip}"
        
        # Property: Geolocation should be structured (Requirement 8.6)
        geolocation = result["geolocation"]
        assert isinstance(geolocation, dict), f"Geolocation data should be a dictionary for IP {ip}"
        
        # Property: ASN info should be structured (Requirement 8.5)
        asn_info = result["asn_info"]
        assert isinstance(asn_info, dict), f"ASN info should be a dictionary for IP {ip}"
        
        # Property: Blocklist results should be a list (Requirements 8.7, 8.8, 8.9)
        blocklist_results = result["blocklist_results"]
        assert isinstance(blocklist_results, list), f"Blocklist results should be a list for IP {ip}"
    ...
    def test_whois_lookup_structure_property(self, ip: IPAddress):
        ...
        **Validates: Requirements 8.3**
        ...
        assert isinstance(whois_data, dict), f"WHOIS data should be dictionary for IP {ip}"
        ...
        if whois_data:  # If not empty
            ...
            expected_whois_fields = [
                "network", "country", "org", "description", "emails", "created", "updated"
            ]
    ...
    def test_reverse_dns_lookup_property(self, ip: IPAddress):
        ...
        **Validates: Requirements 8.4**
        ...
        assert isinstance(whois_data, dict), f"WHOIS data structure should support reverse DNS for IP {ip}"
    ...
    def test_geolocation_data_property(self, ip: IPAddress):
        ...
        **Validates: Requirements 8.6**
        ...
        assert isinstance(geolocation, dict), f"Geolocation should be dictionary for IP {ip}"
    ...
    def test_blocklist_results_structure_property(self, ip: IPAddress):
        ...
        **Validates: Requirements 8.7, 8.8, 8.9 (structure aspect)**
        ...
        assert isinstance(blocklist_results, list), f"Blocklist results should be list for IP {ip}"
