...
class IPAddressHandler:
    ...
    def validate_ip(ip_str: str) -> Union[IPv4Address, IPv6Address]:
        ...
        if not ip_str:
            raise IPAddressValidationError("IP address cannot be empty or whitespace only")
        
        try:
            return ip_address(ip_str)
        except (AddressValueError, ValueError) as e:
            raise IPAddressValidationError(f"Invalid IP address format '{ip_str}': {str(e)}")
    ...
    def is_in_subnet(ip: Union[str, IPv4Address, IPv6Address], 
        ...
        try:
            ...
            raise IPAddressValidationError(f"Invalid subnet format '{subnet}': {str(e)}")
    ...
    def get_network_info(ip: Union[str, IPv4Address, IPv6Address], 
        ...
        if prefix_length is not None:
            try:
                ...
                raise IPAddressValidationError(f"Invalid prefix length {prefix_length}: {str(e)}")
