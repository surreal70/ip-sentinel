...
class DatabaseManager:
    ...
    def migrate_database(self) -> None:
        ...
        if current_version < self.SCHEMA_VERSION:
            self.logger.info(
                f"Migrating database from version {current_version} to {
                    self.SCHEMA_VERSION}")
    ...
    def store_scan_result(self, result) -> int:
        ...
        try:
            ...
            if hasattr(result, 'to_dict'):
                ...
                raise DatabaseError(
                    "Result must be AnalysisResult object or dictionary")
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for module_name, module_result in module_results.items():
                    if not isinstance(module_result, dict):
                        self.logger.warning(
                            f"Invalid module result format for {module_name}")
                        continue
                ...
                for row in rows:
                    ...
                    if row[6]:  # module_name
                        try:
                            ...
                            self.logger.warning(
                                f"Invalid JSON in result data for scan {scan_id}, module {
                                    row[6]}")
                            result_data = {}
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                scan = {
                    ...
                    'modules_executed': json.loads(
                        first_row[5]) if first_row[5] else [],
                    'module_results': {}}
